...
- 가설 (Hypothesis): `BM25 + Dense + Reranker` 조합을 적용하면 정답 근거 회수율이 높아질 것입니다.
- 왜 이 가설이 유효한가 (Rationale): 현재는 Dense only 검색으로 약관 조항형 키워드가 충분히 반영되지 않습니다.
...
- 영향을 받을 수 있는 질의군: 약관, 수수료, 해지 조건, 예외 조항
...
- 예상 리스크: 노이즈 문서 증가
- 회귀 가능 구간: FAQ성 짧은 질의
- 배포 보류 조건: Hallucination Rate 증가 또는 FAQ 회귀 발생
...
- 판단 사유: Top-K Hit Rate는 상승할 가능성이 높으나, 노이즈 비율과 FAQ 질의군 영향 추가 점검이 필요함
