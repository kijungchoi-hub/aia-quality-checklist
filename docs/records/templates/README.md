...
이 폴더는 PM과 프로젝트 리더가 체크리스트와 운영 기준을 실제 품질 운영 문서, 검증 문서, 배포 판단 문서로 연결할 때 사용하는 표준 템플릿 모음입니다.

## 폴더 구조

- [`operations`](operations/): 실험 설계와 실패 사례 축적에 사용하는 품질 운영 템플릿
- [`validation`](validation/): 회귀 검증과 품질 비교에 사용하는 검증 템플릿
- [`release`](release/): 개선 반영 승인과 배포 판단에 사용하는 템플릿
- [`logs`](logs/): 주간 점검과 품질 운영 기록에 사용하는 템플릿
...
6. 주간 품질 운영 기록은 `logs/29_weekly_quality_log.*`로 정리합니다.
...
- 템플릿 작성 결과는 PM과 프로젝트 리더의 품질 판단과 주간 운영 회의의 근거 문서로 활용합니다.
