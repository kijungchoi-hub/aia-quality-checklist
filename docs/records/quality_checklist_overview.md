# 답변 품질 개선활동 개요

기준 문서:
- `docs/checklists/answer_checklist.md`
- `docs/checklists/answer_checklist_table.csv`
- `docs/projects/ai_rag_service_project_tracker.md`

| 항목 | 내용 |
|---|---|
| 작성일 | 2026-04-02 |

## 1. 문서 개요

이 문서는 `answer` 품질 개선 활동을 주요 학습 사항, 실행 과제, 공유 관점에서 재정리한 개선활동 개요 문서입니다.
핵심은 개선 과제 나열이 아니라, 데이터, 검색, 생성, 개선활동을 하나의 흐름으로 보고 우선 실행 항목과 목표 상태를 정리하는 데 있습니다.

## 2. 핵심 제안

1. 품질 개선의 관리 단위를 `체크리스트 항목`에서 `목표 결과` 중심으로 전환합니다.
2. 품질 문제를 `검색 실패`와 `생성 실패`로 분리해 원인 분석 정확도를 높입니다.
3. 모든 개선 활동을 `평가 기준`, `실험 기록`, `회귀 기준`, `배포 게이트`와 연결합니다.
4. 프로젝트별 개선 경험을 공통 코드북과 개선활동 템플릿으로 표준화합니다.

## 3. 목표 결과 중심 정리

| 목표 결과 | 현재 개선점 | 실행 방향 | 판단 기준 |
|---|---|---|---|
| 신뢰 가능한 평가 기반 확보 | 평가셋 정합성과 기준 통일이 완전하지 않음 | 검증 세트 최신화, 정답지와 청크(Chunk) 적합성 보강 | 동일 샘플 기준 전후 비교 가능 |
| 검색 품질의 원인 분해 가능 | 검색 실패 유형이 체계적으로 누적되지 않음 | 실패 코드북 정비, 검색 조합 비교 실험 | 실패 원인이 데이터, 색인, 쿼리, 랭킹으로 분리됨 |
| 생성 품질의 안정적 통제 | 검색 성공 후 생성 실패가 별도 관리되지 않음 | 생성 실패 케이스북 운영, 출처 흐름 검증 | 검색 성공 후 오답 유형이 별도 집계됨 |
| 개선 반영 판단의 일관성 확보 | 회귀 기준과 배포 게이트가 충분히 정착되지 않음 | 회귀 테스트셋, 배포 게이트 기준 정비 | 배포 승인과 보류 사유 설명 가능 |

## 4. 실행 순서

1. `평가 고정`: 평가셋 최신화, 정답지 정리, 청크 적합성 확인
2. `검색 개선`: 정답 근거 회수율 개선, 실패 코드 분류, 검색 조합 비교
3. `생성 개선`: 환각 억제, 근거-답변 연결, 형식 안정화
4. `개선활동 고정`: 회귀 테스트셋 등록, 배포 게이트 정비, 주간 기록 표준화

## 5. 핵심 산출물

| 영역 | 핵심 산출물 |
|---|---|
| 데이터와 평가 | 평가셋, 정합성 점검표, 수동/자동 평가 로그 |
| 검색 | 검색 조합 비교표, 검색 실패 코드북 |
| 생성 | 프롬프트 버전표, 생성 실패 케이스북, 출처 포맷 정의 |
| 개선활동 | 회귀 테스트셋, 배포 게이트 점검표, 주간 품질 점검 기록 |

## 6. 공통 개선활동 원칙

1. 검색 실패와 생성 실패는 항상 분리 관리합니다.
2. 모델 변경 전에는 데이터셋과 검색 기준을 먼저 고정합니다.
3. 모든 개선은 동일 평가셋으로 전후 비교합니다.
4. 배포 전에는 회귀 테스트 결과가 있어야 합니다.
5. 주간 기록은 결과뿐 아니라 실패와 차주 계획까지 포함합니다.

## 7. 권장 연결 문서

- `docs/governance/quality_operating_framework.md`
- `docs/governance/release_gate_checklist.md`
- `docs/records/templates/operations/experiment_design.md`
- `docs/records/templates/validation/regression_test_tracker.md`
- `docs/records/templates/logs/weekly_quality_log.md`
