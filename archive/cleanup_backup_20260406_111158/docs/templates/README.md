...
이 폴더는 `answer` 품질 개선 프로젝트에서 바로 사용할 수 있는 실무형 템플릿 모음입니다.

## 사용 원칙

- CSV/XLSX가 없어도 Markdown 표만으로 운영할 수 있도록 작성했습니다.
- 표의 첫 행은 예시이므로 실제 사용 시 복제하거나 교체해 사용합니다.
- 상태값은 가능하면 `미착수 / 진행중 / 완료 / 보류`로 통일합니다.
- 주간 운영 시에는 파일명을 바꾸기보다 이력을 하단에 누적하거나 복사본을 만들어 관리합니다.
...
1. 평가셋은 `answer_evaluation_dataset_template.md`로 먼저 정비합니다.
2. 실험 중 실패 사례는 검색/생성 케이스북에 분리 누적합니다.
3. 변경 반영 전에는 회귀 테스트셋과 배포 게이트 체크리스트를 반드시 점검합니다.
4. 주간 보고에는 핵심 결과와 회귀, 차주 계획을 동일 형식으로 남깁니다.
